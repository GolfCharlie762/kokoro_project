...
ReLU::ReLU() : input_cache(Tensor()) {} // Explicit default constructor call
...
Tensor ReLU::backward(const Tensor& grad_output, float learning_rate) {
    ...
    for (size_t i = 0; i < size; ++i) {
        grad_input_data[i] = grad_output_data[i] * (input_data[i] > 0 ? 1.0f : 0.0f);
    }
...
std::unique_ptr<ReLU> ReLU::load(std::ifstream& file) {
    auto layer = std::make_unique<ReLU>();
    size_t shape_size;
