#include "max_pooling3d.h"
#include <fstream>
#include <algorithm>

MaxPooling3D::MaxPooling3D(size_t pool_size, size_t stride)
    : pool_size(pool_size), stride(stride), input_cache(Tensor()), max_indices(Tensor()) {}

Tensor MaxPooling3D::forward(const Tensor& input) {
    if (input.shape().size() != 5) {
        throw std::invalid_argument("Input must be 5D with shape {batch_size, channels, depth, height, width}");
    }
    input_cache = input;
    ...
    size_t output_depth = (input_depth - pool_size) / stride + 1;
    size_t output_height = (input_height - pool_size) / stride + 1;
    size_t output_width = (input_width - pool_size) / stride + 1;

    Tensor output({batch_size, channels, output_depth, output_height, output_width});
    max_indices = Tensor({batch_size, channels, output_depth, output_height, output_width});
...
Tensor MaxPooling3D::backward(const Tensor& grad_output, float learning_rate) {
    if (grad_output.shape().size() != 5 ||
        grad_output.shape()[2] != (input_cache.shape()[2] - pool_size) / stride + 1) {
...
void MaxPooling3D::save(std::ofstream& file) const {
...
std::unique_ptr<MaxPooling3D> MaxPooling3D::load(std::ifstream& file) {
    ...
    auto layer = std::make_unique<MaxPooling3D>(pool_size, stride);
    size_t shape_size;
...
void MaxPooling3D::print() const {
    std::cout << "MaxPooling3D Layer: pool_size=" << pool_size << ", stride=" << stride << "\n";
