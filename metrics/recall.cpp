...
float Recall::calculate(const Tensor& predictions, const Tensor& targets) const {
    ...
    return static_cast<float>(true_positives) / (true_positives + false_negatives + 1e-15f);
}

float Recall::calculate_multiclass(const Tensor& predictions, const Tensor& targets) const {
    ...
    for (size_t i = 0; i < samples; ++i) {
        ...
        else false_negatives[targ_idx]++;
    }

    float macro_recall = 0.0f;
    for (size_t c = 0; c < classes; ++c) {
        macro_recall += static_cast<float>(true_positives[c]) / (true_positives[c] + false_negatives[c] + 1e-15f);
    }

    return macro_recall / classes;
}
